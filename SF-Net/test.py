...
def test(itr, dataset, args, model, logger, device):
    ...
    print(cmap)
    # with open('detial.txt','a') as f:
    #     f.write('Classification map %f\n' %cmap)
    dmap, iou = dmAP(tcam_stack, dataset.path_to_annotations, args.activity_net, dataset, valid_id=dataset.lst_valid)
    ...
    for k in range(len(dmap)):
        ...
        # with open('/detial.txt',"a") as f:
        #     f.write('Detection map @ %f = %f\n' %(iou[k], dmap[k]))
        
    
    print('Mean Detection map = %f' %(np.mean(dmap)))
    # with open('/detial.txt',"a") as f:
    #     f.write('Mean Detection map = %f\n' %(np.mean(dmap)))
    dmap += [np.mean(dmap)]    
